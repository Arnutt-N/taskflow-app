...
function useBreadcrumb(pathname: string) {
  ...
  for (const seg of segments) {
    ...
    if (label.toLowerCase() === 'audit log' || label.toLowerCase() === 'audit logs') {
        label = 'Audit Log';
    }
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-6 sidebar-scroll">
          ...
          {/* Moved Collapse toggle to bottom of nav */}
          <div className={`mt-auto pt-4 ${collapsed ? 'flex justify-center' : 'px-3'}`}>
             <button
              onClick={toggleCollapse}
              className={`flex items-center gap-3 p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors w-full ${collapsed ? 'justify-center' : ''}`}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* ── Top Navbar ── */}
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-4 gap-4 flex-shrink-0 shadow-sm shadow-slate-100 relative">
          ...
          {/* Center: Search Bar (Absolute centered on large screens) */}
          <div className="hidden md:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md">
            ...
            >
               ...
               <kbd className="hidden md:inline bg-white border border-slate-200 text-slate-500 text-[10px] font-mono px-1.5 py-0.5 rounded font-medium">⌘K</kbd>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => { setSearchOpen(true); setTimeout(() => searchRef.current?.focus(), 50); }}
              className="md:hidden flex items-center gap-2 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-400 text-xs transition-colors"
            >
      ...
      {/* ── Cmd+K Search Modal ── */}
      {searchOpen && (
        <div className="fixed inset-0 z-[60] flex items-start justify-center pt-[15vh] px-4 font-sans">
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={() => { setSearchOpen(false); setSearchQuery(''); }} />
          <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden z-10 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-4 px-6 py-5 border-b border-slate-100 bg-slate-50/50">
              ...
              <button 
                ...
                className="group flex items-center justify-center w-8 h-8 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200"
                aria-label="Close search"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="px-6 py-4 bg-white text-xs text-slate-400 flex items-center justify-between border-b border-slate-50">
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5"><kbd className="bg-slate-100 border border-slate-200 px-2 py-0.5 rounded font-mono text-[10px] font-bold text-slate-500">↵</kbd> to search</span>
                <span className="flex items-center gap-1.5"><kbd className="bg-slate-100 border border-slate-200 px-2 py-0.5 rounded font-mono text-[10px] font-bold text-slate-500">Esc</kbd> to close</span>
              </div>
            </div>
            {/* Quick links */}
